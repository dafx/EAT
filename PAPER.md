...
| CNN encoder / patch stem for audio | [models/images.py](models/images.py#L61), [models/images.py](models/images.py#L89) | The audio encoder is implemented in `ImageEncoder`; for 1-channel audio it builds the patch stem with `PatchEmbed_new(...)` on the spectrogram input. |
